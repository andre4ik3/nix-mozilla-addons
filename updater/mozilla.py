...
from updater import to_sri_hash_prefixed, get_extra_addons
...
def get_addon(http: PoolManager, api_base: str, addon_id: str, name: str) -> dict:
    ...
    return {
        ...
        "file": {
            "url": file["url"],
            "hash": to_sri_hash_prefixed(file["hash"]),
        },
        "passthru": {
            "id": data["guid"],
            "alias": data["slug"],
...
def update_addons_for_mozilla_product(http: PoolManager, product: str, addons: dict[str, str]):
    """Updates all addons for a particular Mozilla-like product."""
