...
import { fetch, response, parseJSON, parseSchema, toSRI, Addon, hash, tupleOf, digestToSRI } from "./util.ts";
...
export const fetchAMO = (server: keyof typeof SERVERS, id: string) => fetch(`${SERVERS[server]}/v4/addons/addon/${id}/`).pipe(
...
const Manifest = z.object({
  addons: z.record(z.string(), z.object({
...
export const fetchMozilla = (url: string) => fetch(url).pipe(
  Effect.andThen(response.ok),
  Effect.andThen(response.text),
  Effect.andThen(parseJSON),
  Effect.andThen(parseSchema(Manifest)),
  Effect.map((data) => Object.entries(data.addons).map(([id, { updates }]) => ({
    id,
